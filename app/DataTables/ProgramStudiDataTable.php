        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($item) {
                ...
                if (Gate::check('program_studi_edit')) {
                    $btn .= '<a href="' . route('programStudi.edit', $item->id) . '" class="btn btn-sm btn-warning text-white rounded shadow-sm d-flex align-items-center justify-content-center" style="width: 30px; height: 30px;" title="Edit"><i class="fa-solid fa-pen-to-square" style="font-size: 11px;"></i></a>';
                }
                if (Gate::check('program_studi_delete')) {
                    $btn .= '<form action="' . route('programStudi.destroy', $item->id) . '" method="POST" class="m-0">' . csrf_field() . method_field('delete') . '<button type="submit" class="btn btn-danger btn-sm rounded shadow-sm d-flex align-items-center justify-content-center" style="width: 30px; height: 30px;" title="Hapus" onclick="return confirm(\'Hapus data ini?\')"><i class="fa-solid fa-trash-can" style="font-size: 11px;"></i></button></form>';
                }
                $btn .= '</div>';
                
                if (Gate::check('program_studi_edit') || Gate::check('program_studi_delete')) {
                    return $btn;
                }
